# -*- coding: utf-8 -*-
"""
Created on Sun Jan 16 00:17:04 2022
...
Uncer_freq_start, Uncer_freq_end = 191.5e12, 192e12
...
class AbsoluteDistance():
    ...
    def path_difference(self, F_unequal, I_unequal, cutT=10e-12, cutwidth=1e-12, expnum=16, pad_exp=3, ana_freq_start=191.65e12, ana_freq_end=191.75e12):
        ...
        self.I_han_pad, self.acf_pad = self.zero_padding(
            self.I_han, self.len_pad)
        ...
        self.F_pad_cut, self.phi_cut = self.F_pad[(ana_freq_start < self.F_pad) & (
            self.F_pad < ana_freq_end)], self.phi[(ana_freq_start < self.F_pad) & (self.F_pad < ana_freq_end)]
        self.a, self.b = np.polyfit(
            self.F_pad_cut, self.phi_cut, 1)  # phi = a *F + bの1じ多項式近似
        self.F_pad_Uncer, self.phi_Uncer = self.F_pad[(Uncer_freq_start < self.F_pad) & (
            self.F_pad < Uncer_freq_end)], self.phi[(Uncer_freq_start < self.F_pad) & (self.F_pad < Uncer_freq_end)]

        # https://biotech-lab.org/articles/4907 R2値
        self.R2 = metrics.r2_score(
            self.phi_cut, self.a * self.F_pad_cut + self.b)
...
df_phi_Uncer = pd.DataFrame(columns=names_rawdata)
...
for idict_Params in LIST_HYPERPARAMS:
    ...
    *2 データ保存
    ...
    dir_Ana = os.path.join(os.path.split(matomexlsxpath)[
                           0], "AnaResults_F_pad_limit_Ver")
    ...
    name_file_AnaResult = "cutT" + str(idict_Params["cutT"]) + "_" +\
        ...
        "startend"+str(idict_Params["ANA_FREQ_START"]) + \
        "-"+str(idict_Params["ANA_FREQ_END"]) + ".xlsx"
...
df_phi_Uncer.to_excel(
    "//ark-3/common_working/90)受け渡しフォルダ/受け渡しフォルダ_2021/廣田→廣田/uncer/inter\phi.xlsx")
df_phi_Uncer.to_excel("phi_Uncer.xlsx")
