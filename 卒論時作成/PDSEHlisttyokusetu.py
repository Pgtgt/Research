...
def traia_line(F,wrap):#０－３．１４の三角波を直線に
    ...
    mountafterindex1,mountafterindex2,valleyafterindex1,valleyafterindex2=[],[],[],[]


    for i in range(len_wrap-1):
        if (((np.diff(wrap)[i-1]>=0)&(np.diff(wrap)[i]<0)) & (wrap[i-1]<=wrap[i+1])):
            mountafterindex1.append(i+1)    
        elif (((np.diff(wrap)[i-1]>=0)&(np.diff(wrap)[i]<0)) & (wrap[i-1]>wrap[i+1])):
            mountafterindex2.append(i)
        elif ((np.diff(wrap)[i-1]<0)&(np.diff(wrap)[i]>=0)) & (wrap[i-1]>=wrap[i+1]):
            valleyafterindex1.append(i+1)
        elif ((np.diff(wrap)[i-1]<0)&(np.diff(wrap)[i]>=0)) & (wrap[i-1]<wrap[i+1]):
            valleyafterindex2.append(i)
    t2=time.time()
    ...
    t2=time.time()
    print("Peak index timesort",t2-t1)
