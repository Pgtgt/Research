...
def traia_line(F,wrap):#０－３．１４の三角波を直線に
    ...
    mountafterindex1 = [i+1 for i in range(1,len_wrap-1) \
              if ( ((diff_wrap[i-1]>=0)&(diff_wrap[i]<0))) & (wrap[i-1]<=wrap[i+1])]
    mountafterindex2 = [i for i in range(1,len_wrap-1) \
              if ( ((diff_wrap[i-1]>=0)&(diff_wrap[i]<0))) & (wrap[i-1]>wrap[i+1])]
    mountafterindex = mountafterindex1 + mountafterindex2  
    mountafterindex=sorted(mountafterindex) #小さいインデックスが上に
    valleyafterindex1 = [i+1 for i in range(1,len_wrap-1) \
              if ( ((diff_wrap[i-1]<0)&(diff_wrap[i]>=0))) & (wrap[i-1]>=wrap[i+1])]
    valleyafterindex2 = [i for i in range(1,len_wrap-1) \
              if ( ((diff_wrap[i-1]<0)&(diff_wrap[i]>=0))) & (wrap[i-1]<wrap[i+1])]
    valleyafterindex = valleyafterindex1 + valleyafterindex2
    valleyafterindex=sorted(valleyafterindex) 
    
    t2=time.time()
    print("Peak index time",t2-t1)
    ...
    if (len(mountafterindex)==len(valleyafterindex)):#/\/\/
        ...
        for _ in range(len_change):#/\/\/まで一括処理
            ...
            =-(wrap[mountafterindex[_]:valleyafterindex[_]]-math.pi)  +  (2*_)*math.pi
...
file=FilefromDia()
# file="F:\研究\Data20191205\TenU\A0011.CSV"   
